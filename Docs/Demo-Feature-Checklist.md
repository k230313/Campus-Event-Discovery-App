...
## 14. Deployment / Run Guide for Demo

There are two practical ways to demonstrate CEDA:

### Option A: Use the live deployed website

This is the easiest option if the live site is healthy and internet is available.

Use this option when:

- the public deployment is stable
- email features need live external services
- the team wants the least setup risk during the presentation
...
This is the fallback or classroom-friendly option when the teacher wants the source code and a runnable local copy.
...
After download/extract, open the project root:
...
- `MySQL Workbench`
  - for viewing and importing the database
- `Git`
  - only needed if cloning from GitHub instead of downloading ZIP
...
At minimum, the local backend needs the database and auth settings.
...
In the project root, install dependencies in both app folders:
...
You can start it in either way:
...
Or on Windows use the helper launcher:

- [run-local-demo.bat](/C:/Users/Admin/Campus-Event-Discovery-App/run-local-demo.bat)

That batch file:

- opens one terminal for backend
- opens one terminal for frontend
...
#### 7. What requires internet even in local mode
...
- `schema.sql`
- setup instructions
- a sanitized `.env.example` if you create one later
